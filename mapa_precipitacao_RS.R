# Usando como base esse vídeo: https://youtu.be/vYvuhxUdHac?t=7245
...
# Cargammos los SHp do Brasil ---------------------------------------------------------------
Brasil   <- getData('GADM', 
                    country='Brazil',
                    level=3) %>%
  st_as_sf() 

MDD         <-subset(Brasil , NAME_1 == "Rio Grande do Sul") 

lbl         <- data.frame(month_abb = month.abb, mes = 1:12)

MDD_xy <- cbind(
  MDD, st_coordinates(
    st_centroid(MDD$geometry
    )
  )
)
...
gg <- ggplot(vls)  +
  ...
  labs(title = 'Precipitação Mensal - RS', fill = 'mm',  x = 'Longitude', y = 'Latitude', caption = "Leonardo Fernandes") +
